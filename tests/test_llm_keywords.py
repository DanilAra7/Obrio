...
from eval.llm_keywords import _verify


def test_verify_keeps_real_citations_and_recomputes_count():
    phrases = [{"phrase": "charged twice", "mention_count": 5, "review_ids": ["1", "2"],
               "example_quote": "..."}]
    out = _verify(phrases, valid_ids={"1", "2", "3"})
    assert out[0]["review_ids"] == ["1", "2"]
    assert out[0]["mention_count"] == 2  # recomputed from verified ids, not trusted as 5
...
def test_verify_drops_hallucinated_ids_and_recomputes_count():
    phrases = [{"phrase": "charged twice", "mention_count": 3, "review_ids": ["1", "999"],
               "example_quote": "..."}]
    out = _verify(phrases, valid_ids={"1", "2"})
    assert out[0]["review_ids"] == ["1"]
...
def test_verify_sorts_by_recomputed_count_descending():
    ...
    out = _verify(phrases, valid_ids={"1", "2", "3"})
    assert [p["phrase"] for p in out] == ["b", "a"]
